    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (!Array.isArray(entries) || entries.length === 0) {
                console.error(
                    'Unexpected shop structure:',
                    JSON.stringify(json, null, 2)
                );
                return interaction.editReply(
                    '❌ Could not fetch any items from the shop.'
                );
            }
            ...
            const buildEmbed = (entry, i) => {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setColor('#00AEEF')
                    .setFooter({
                        text: `Item ${i + 1} of ${entries.length} • Powered by fortnite-api.com`,
                    })
                    .setTimestamp();
            ...
            await interaction.editReply(
                '❌ Something went wrong retrieving the shop.'
            );
        }
    },
};
